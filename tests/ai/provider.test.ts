...
function mockCompletion(content:unknown,finish='stop'){const fetcher=vi.fn().mockImplementation(async()=>new Response(JSON.stringify({choices:[{message:{content:typeof content==='string'?content:JSON.stringify(content)},finish_reason:finish}]}),{status:200}));vi.stubGlobal('fetch',fetcher);vi.stubEnv('OPENROUTER_API_KEY','test-key-never-display');return fetcher;}
