...
export function generateProposal(state: AppState, lessonId: string, raw: z.input<typeof generateProposalSchema> & { provenance: Provenance; changes?: ProposalChange[] }, ctx: DomainContext = {}): Proposal {
  ...
  const defaultChanges: ProposalChange[]=[
    {...common,id:id('change',ctx),operation:'replace_practice',rationale:'Use the reviewed work to give targeted help, continued application and extension during the same 12 minutes. Unresolved evidence receives a short check.',payload:{block:{id:'practice',title:'Practice groups',minutes:12,mode:'concurrent',instructions:'Run these activities concurrently. Keep the rest of the lesson on schedule; record any help given.',lanes:[
      {id:'targeted',title:'Equal-parts practice',studentIds:targeted,teacherLed:true,minutes:12,instructions:'Use equal-length fraction strips. Rename each addend in the same-sized parts, then add. Ask why adding denominators changes the unit.',materialIds:['targeted-equal-parts-v1'],entryCheckStudentIds:[]},
      ...
      {id:'extension',title:'Compare two methods',studentIds:extension,teacherLed:false,minutes:12,instructions:'Solve with two valid common denominators and explain why the results name the same quantity.',materialIds:['extension-explain-v1'],entryCheckStudentIds:[]},
    ]}}},
    {...common,id:id('change',ctx),operation:'replace_exit',rationale:'Check whether students can explain why fractions need equal-sized parts.',payload:{block:{id:'exit',title:'Explain the common unit',minutes:5,instructions:'Solve 1/4 + 1/6 and finish: Before adding, I rewrite the fractions because… Record actual assistance.',mode:'whole_class',materialIds:['exit-equal-units-v1']}}},
  ];
  for (const checkpoint of getEligibleCheckpoints(state,lessonId).slice(0,1)) defaultChanges.push({...common,id:id('change',ctx),operation:'schedule_checkpoint',rationale:'Add a short check during the next lesson. Keep assessment and unit dates fixed.',payload:checkpoint});
