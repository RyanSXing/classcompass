...
function safeFailure(status: number, body: unknown, retryAfter: string | null, provider: 'openrouter' | 'deepseek'): AIError {
  ...
  if (status === 401 || status === 403) return new AIError('AI_AUTH', `${providerName} did not authorize this request. Check the server API key and account/model access.`, false);
  if (status === 402 || /daily|per.day|credits|quota.*exhaust/.test(text)) return new AIError('AI_QUOTA', provider === 'deepseek' ? 'The DeepSeek account balance or quota is unavailable. Check the account before retrying; no fallback provider was used.' : 'The OpenRouter free allowance is exhausted or unavailable. Check the account allowance before retrying; no paid fallback was used.', false);
  if (status === 429) { const seconds = retryAfter && /^\d+(\.\d+)?$/.test(retryAfter) ? Number(retryAfter) * 1000 : retryAfter ? Date.parse(retryAfter) - Date.now() : 5000; return new AIError('AI_RATE_LIMIT', `${providerName} is rate limited. This step can retry after the provider wait.`, true, Math.max(4000, Math.min(Number.isFinite(seconds) ? seconds : 5000, 86400000))); }
  if (status === 413) return new AIError('AI_CONTEXT_LIMIT', 'The provider rejected this request because its classroom context is too large. Your work is saved. A provider with a larger request limit is needed.', false);
  if (status === 400 || status === 404 || status === 422) return new AIError('AI_MODEL_UNAVAILABLE', `The configured ${providerName} model or required image/JSON parameters are unavailable. Check model availability; no model was substituted.`, false);
  ...
  return new AIError('AI_PROVIDER', `${providerName} could not complete this step. The saved work is unchanged; retry when the provider recovers.`, status >= 500);
}

/** Exactly one network call; jobs own pacing, caching, retries and cancellation. */
export async function completion(model: string, messages: unknown[], responseFormat: unknown, maxTokens: number, textReasoning?: TextReasoning): Promise<unknown> {
  const config = configuration(), provider = config.aiProvider;
  if (provider === 'openrouter' && !model.endsWith(':free')) throw new AIError('AI_MODEL_COST', 'Only explicitly configured free OpenRouter endpoints are enabled. No paid request was made.', false);
  const keyName = provider === 'deepseek' ? 'DEEPSEEK_API_KEY' : 'OPENROUTER_API_KEY';
  const key = process.env[keyName]?.trim();
  if (!key) throw new AIError('AI_KEY_MISSING', `Set ${keyName} on the server to use the selected live provider, or choose the disclosed sample mode.`, false);
  const format = responseFormat as { type?: string; json_schema?: { schema?: unknown } };
  // Direct DeepSeek supports JSON mode, not the OpenRouter json_schema parameter.
  // Keep the same schema in the system prompt; each caller still validates it with Zod.
  const providerMessages = provider === 'deepseek' ? [{ role: 'system', content: format.json_schema?.schema ? `Return only a JSON object satisfying this JSON Schema exactly. Do not add Markdown or commentary: ${JSON.stringify(format.json_schema.schema)}` : 'Return only a JSON object matching the requested output shape. Do not add Markdown or commentary.' }, ...messages] : messages;
  const requestBody = provider === 'deepseek'
    ? { model, messages: providerMessages, stream: false, max_tokens: maxTokens, response_format: { type: 'json_object' }, thinking: { type: textReasoning === 'low' ? 'enabled' : 'disabled' }, ...(textReasoning === 'low' ? { reasoning_effort: 'low' } : { temperature: 0 }) }
    : { model, messages, stream: false, max_tokens: maxTokens, response_format: responseFormat, provider: { require_parameters: true }, ...(textReasoning ? { reasoning: textReasoning === 'low' ? { effort: 'low' } : { enabled: false } } : {}) };
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), config.providerTimeout);
  try {
    const response = await fetch(ENDPOINTS[provider], { method: 'POST', headers: { Authorization: `Bearer ${key}`, 'Content-Type': 'application/json', ...(provider === 'openrouter' ? { 'X-OpenRouter-Title': 'ClassCompass' } : {}) }, body: JSON.stringify(requestBody), signal: controller.signal, redirect: 'error' });
    ...
    let body: unknown;
    try { body = JSON.parse(text); } catch { if (!response.ok) throw safeFailure(response.status, null, response.headers.get('retry-after'), provider); throw new AIError('AI_INVALID_JSON', 'The provider did not return a complete JSON response. Retry this step.', true); }
    if (!response.ok) throw safeFailure(response.status, body, response.headers.get('retry-after'), provider);
    ...
    if ('error' in body) { const inner = (body as {error:{code?:number}}).error; throw safeFailure(typeof inner?.code === 'number' ? inner.code : 502, body, response.headers.get('retry-after'), provider); }
    ...
    throw new AIError('AI_NETWORK', `Could not connect to ${provider === 'deepseek' ? 'DeepSeek' : 'OpenRouter'}. Check network access and retry this step.`, true);
