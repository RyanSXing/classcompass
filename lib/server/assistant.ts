...
import { currentAssignmentFindings, getAssignmentAnalytics, selectResponseRevision } from '@/lib/analytics';
...
export function buildAssistantContext(state: AppState, scope: AssistantScope = {}, excludeRequestId?: string) {
  ...
  const understanding = getUnderstandingOverview(state, assignments.at(-1)!.templateId);
  const skillUnderstanding = {
    legend: understanding.legend, rules: understanding.rules,
    students: understanding.students.map(student => ({ studentId: student.studentId, skills: student.skills.map(skill => ({ skillId: skill.skillId, label: skill.label, cells: skill.cells.map(cell => ({
      templateId: cell.templateId, date: cell.date, stage: cell.stage, reason: cell.reason, nextStep: cell.nextStep,
      evidence: [...cell.supportingEvidence, ...cell.counterEvidence, ...cell.otherEvidence].map(ref => ({ sourceId: `response:${ref.responseId}:${ref.responseRevision}`, role: ref.role, note: ref.note, date: ref.activityDate, help: ref.support.level })),
    })) })) })),
  };
  const classroom = { catalogVersion: CATALOG_VERSION, name: state.classroom.name, grade: state.classroom.grade, subject: state.classroom.subject, unit: curriculum.unit, objectives: curriculum.objectives, criteria: curriculum.criteria, students, assignments: work, learningDevelopment, skillUnderstanding, currentNotes: projectedNotes, historicalNotes: projectedHistory, historicalResponses, plans, historicalPlans, currentMaterialSets, availableMaterials: curriculum.materials, calendar, goals };
...
export function buildAssistantModelContext(context: ReturnType<typeof buildAssistantContext>, provider = configuration().aiProvider, message = '') {
  ...
  return {
    classroom: {
      ...
      skillUnderstanding: {
        throughDate: focusAssignment.date,
        legend: c.skillUnderstanding.legend.map(({stage,label}) => ({stage,label})),
        meaning: 'Rule-based stages of the shown skill, using methods and recorded help. Not scores, diagnoses or permanent labels. Evidence sources below retain the actual work and help conditions.',
        columns: ['studentId', 'skill', 'stage', 'reason', 'nextStep', 'supportingSources', 'counterSources', 'otherSources'],
        rows: c.skillUnderstanding.students.flatMap(student => student.skills.flatMap(skill => {
          const cell = skill.cells.find(cell => cell.templateId === focusAssignment.templateId);
          return cell ? [[student.studentId, skill.label, cell.stage, cell.reason, cell.nextStep, ...(['supporting','counter','context'] as const).map(role => cell.evidence.filter(ref => ref.role === role).map(ref => alias(ref.sourceId)))]] : [];
        })),
      },
...
function sampleLearningReply(context: ReturnType<typeof buildAssistantContext>, templateId: string, studentId?: string): AssistantOutput {
  ...
  const next = followUps.map(followUp => {
    ...
    actions.push({ title: `${names([followUp.studentId])}: ${followUp.title}`, description: `Time: ${followUp.minutes} min\nWith: ${names([followUp.studentId])}\nDo: ${followUp.steps[0]}\nCheck: ${followUp.successCheck}`.slice(0, 600), sourceId });
    return `${names([followUp.studentId])}: ${followUp.title}.`;
  }).join('\n\n');
  ...
  const caveat = trend ? trend.comparability : 'Check another task and record any help before judging progress.';
  ...
  answer += '\n\nSample guidance; no live model was called. No lesson was changed.';
...
function sampleReply(context: ReturnType<typeof buildAssistantContext>, message: string, kind: 'chat' | 'brief'): AssistantOutput {
  ...
  const rows = selected.responses.filter(r => !studentId || r.studentId === studentId);
  ...
  const teachingText = chosen.map(action => `${action.studentIds.filter(id => !studentId || id === studentId).map(studentName).join(', ')}: ${action.title}. ${action.status === 'needs_reading' ? 'Check the reading first.' : ''}`).join('\n');
  const intro = `${student ? `${student.displayName} · ` : ''}${selected.title}: ${actionFacts[0]?.reason ?? 'Look at the working and recorded help before choosing the next step.'}`;
  ...
  const actions = chosen.map(action => ({ title: action.title, description: `Time: ${action.minutes} min\nDo: ${action.steps[0]}\nCheck: ${action.successCheck}`.slice(0, 600), sourceId: actionSource(action) }));
...
/** Narrow fact check for explicit single-student unfinished-work totals. */
function unfinishedTotalContradictions(answer: string, context: ReturnType<typeof buildAssistantContext>) {
  const numberWords = ['zero', 'one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine', 'ten'];
  const count = String.raw`(\d+|${numberWords.join('|')})`;
  const datePattern = String.raw`(?:Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)\.?\s+\d{1,2}(?:st|nd|rd|th)?`;
  const status = String.raw`(?:blank|unfinished|unanswered)`;
  const noun = String.raw`(?:answers?|responses?|questions?)`;
  const assignments = context.classroom.assignments;
  const focused = assignments.find(item => item.templateId === context.scope.templateId) ?? [...assignments].reverse().find(item => item.responses.some(row => row.sourceId));
  const contradictions: { student: string; claim: string; date: string; actualUnfinished: number; questions: number[] }[] = [];
  for (const sentence of answer.split(/(?<=[!?])\s+|(?<=\.)\s+(?=[A-Z])|\n+/)) {
    const students = context.classroom.students.filter(student => new RegExp(`\\b${escapeRegExp(student.displayName.split(' ')[0])}\\b`, 'i').test(sentence));
    // Collective, conditional and multi-date statements need interpretation;
    // do not pretend this limited check can establish their meaning.
    if (students.length !== 1 || /\b(?:if|whether|might|may|could)\b/i.test(sentence)) continue;
    const student = students[0];
    const claim = new RegExp(String.raw`\b${escapeRegExp(student.displayName.split(' ')[0])}\s+(?:has|had|left)\s+${count}\s+(?:${datePattern}\s+)?(?:${status}\s+${noun}|${noun}\s+(?:(?:are|were|was)\s+)?${status})\b`, 'i').exec(sentence);
    if (!claim) continue;
    const dates = assignments.filter(assignment => {
      const date = new Date(`${assignment.date}T12:00:00Z`);
      ...
      return sentence.includes(assignment.date) || new RegExp(`\\b(?:${short}|${long})\\.?\\s*${Number(assignment.date.slice(8))}(?:st|nd|rd|th)?\\b`, 'i').test(sentence);
    });
    if (dates.length > 1 || (!dates.length && new RegExp(`${datePattern}|\\d{4}-\\d{2}-\\d{2}`, 'i').test(sentence))) continue;
    const assignment = dates[0] ?? focused;
    if (!assignment) continue;
    const rows = assignment.responses.filter(row => row.studentId === student.id);
    if (rows.some(row => ['unprocessed', 'not_received', 'flagged'].includes(row.result))) continue;
    const unfinished = rows.filter(row => row.result === 'unanswered');
    const stated = /^\d+$/.test(claim[1]) ? Number(claim[1]) : numberWords.indexOf(claim[1].toLowerCase());
    if (stated !== unfinished.length) contradictions.push({ student: student.displayName, claim: claim[0], date: assignment.date, actualUnfinished: unfinished.length, questions: unfinished.map(row => assignment.questions.findIndex(question => question.id === row.questionId) + 1) });
  }
...
function validateOutput(raw: unknown, context: ReturnType<typeof buildAssistantContext>, maxActions = 5, rosterOnly = false, maxWords?: number, learningBrief = false) {
  ...
  if (learningBrief) {
    const contradictions = unfinishedTotalContradictions(output.answer, context);
    if (contradictions.length) issues.push(new AssistantValidationError('AI_BRIEF_ANSWER_TOTALS', 'The briefing misstated a student’s unfinished work.', { instruction: 'Correct the claim using the saved work below. Prefer naming the particular unfinished question and the next teaching step instead of reporting a total.', contradictions }));
  }
...
async function generate(repo: Repository, input: AssistantRequestInput | ClassroomBriefInput, kind: 'chat' | 'brief') {
  ...
  try {
    ...
    const replyLimits = { maxActions: rosterOnly ? 0 : kind === 'brief' ? 3 : 2, maxSourceIds: citationLimit, maxAnswerWords: maxWords, ...(kind === 'brief' ? { maxOpeningWords: 35, maxComparisons: 2, comparisonSourcesMustBeSelected: true } : {}) };
