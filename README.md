...
For direct DeepSeek, set `AI_PROVIDER=deepseek`, add `DEEPSEEK_API_KEY` to `.env.local`, and set `AI_MODE=live` for worksheet processing. Both `DEEPSEEK_VISION_MODEL` and `DEEPSEEK_REASONING_MODEL` default to `deepseek-flash`. Direct calls are billed to the configured DeepSeek account. A two-call live smoke check read all four answers on one fictional worksheet correctly and produced a student analysis that passed domain validation. This is a limited check, not a whole-class accuracy claim; see [verification results](docs/verification-results.md) for the full current evaluation.
