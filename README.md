...
For live analysis, place `OPENROUTER_API_KEY` in `.env.local` and choose `AI_MODE=live`. The configured free models are Gemma for transcription and DeepSeek for analysis/planning. Provider failures stay visible; there is no silent fixture or paid-model fallback. Initial live checks returned Gemma rate limits and a DeepSeek timeout, so **live handwriting accuracy has not been measured**. Run `npm run test:live` when the providers are available.
