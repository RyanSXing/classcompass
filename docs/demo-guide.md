...
For an upload scene, use **Upload work → First check → Load sample worksheets → Analyze this upload** in a clean workspace. This analysis follows `AI_MODE`; set `AI_MODE=fixture` and restart the server for prepared readings. **Load sample class** explicitly uses prepared readings regardless of that setting and is faster for showing all five dates. Prepared readings must not be presented as live OCR. Ordinary uploads still accept the supplied PNGs, JPEGs and one-page PDFs.
...
| 0:25–0:55 | Learning picture → Adding fractions → Casey’s dated squares | Show the stage chart from earlier help to later independent working. Open **Show me why** for the actual responses. Explain that stages describe the work, not a grade. |
| 0:55–1:20 | Choose Equivalent fractions, then Adding fractions for Devon | The renaming is sound; the addition needs a check. Switch to Explaining and applying to show a gap where that skill was not assessed. Detailed answer counts remain under **Explore the evidence**. |
...
| 2:40–4:15 | October 1 lesson → Review suggestions; inspect evidence, edit an instruction and save selected changes | Show the Current / Suggested choice, then the saved 45-minute timeline and Do / Ask / Check cards. Open a worked example or the printable teacher plan. |
| 4:15–4:40 | Overview → Open saved lesson; refresh the briefing if it needs updating | A saved lesson stays saved. Corrected evidence updates the context while preserving earlier readings, help and lesson versions. Correcting historical work need not change the next teaching recommendation when later work already supports it. |
