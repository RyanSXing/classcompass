...
`/classroom` leads with a teaching brief and evidence-linked instructional actions. The Class results, Students over time, and Question patterns tabs expose the same effective responses through different views. A comparison chooser pairs students' usable independent core work across two assignments and shows changes in help and task mix. Numerical correctness, visible method, units, missing answers and uncertain readings remain separate. `lib/insights.ts` derives starting activities from these facts; these cards are labeled as data-based suggestions, not model output.
...
The server assembles current effective work for all five assignments, recorded help, answer criteria, current reviews, bounded historical notes/plans, saved lesson instructions, materials, goals and calendar constraints. Recent conversation is included with a visible context disclosure. Direct DeepSeek receives explicit named-object classroom context, including full teacher guides. The free OpenRouter path uses a disclosed compact projection. Source IDs are constrained by a request-specific schema and resolved to canonical evidence links by the server. Model-authored URLs are rejected. Named-student claims must cite that student's response or review, but this guard is not a proof of every natural-language claim. Teachers can inspect the actual source and approve changes through the existing review workflow.
...
Chat and brief requests use a stable request ID. A failed request can be retried without duplicating its user turn. Provider calls occur outside transactions; changed classroom evidence or goals invalidate a pending result. Old briefs remain saved and are visibly stale when their context changes. Chat cannot confirm a finding, edit a lesson, move a deadline or write another owner's records. Optional assistant state lives in the existing owner-scoped classroom header, with migration `202609190004_classroom_assistant.sql`; no new public data store or service credential is exposed. The explicit local demo reset also clears goals, conversations and briefs.
