...
async function main() {
  ...
  try {
    ...
    const assistantRepo=new SupabaseRepository({id:ownerA,name:'Temporary assistant check',client:a});
    const beforeAssistant=await assistantRepo.read();
    await saveTeacherGoals(assistantRepo,{text:'Temporary fictional goal: collect independent explanations.',expectedRevision:0});
    const chat=await askClassroomAssistant(assistantRepo,{message:'What should I teach next?',requestId:'supabase-assistant-chat',mode:'fixture'});
    const repeated=await askClassroomAssistant(assistantRepo,{message:'What should I teach next?',requestId:'supabase-assistant-chat',mode:'fixture'});
    assert.equal(repeated.turn.id,chat.turn.id);assert.equal(repeated.reused,true);
    await generateClassroomBrief(assistantRepo,{requestId:'supabase-assistant-brief',scope:{templateId:'baseline-template-v1'},mode:'fixture'});
    const assistantLoaded=await assistantRepo.read(),assistant=getAssistantState(assistantLoaded);
    assert.equal(assistant.goals.revision,1);assert.equal(assistant.turns.length,2);assert.equal(assistant.briefs.length,1);assert.equal(assistant.brief?.stale,false);
    assert.deepEqual(assistantLoaded.planVersions,beforeAssistant.planVersions);assert.deepEqual(assistantLoaded.findings,beforeAssistant.findings);
    assert.equal(((await b.rpc('load_classcompass_state')).data as AppState).assistant,undefined);
    const assistantOwnerInjection=structuredClone(assistantLoaded);assistantOwnerInjection.revision++;
    (assistantOwnerInjection.assistant!.turns[0] as unknown as {ownerId:string}).ownerId=ownerB;
    assert.equal((await a.rpc('commit_classcompass_state',{p_expected_revision:assistantLoaded.revision,p_state:assistantOwnerInjection})).error?.code,'42501');
    await saveTeacherGoals(assistantRepo,{text:'Revised temporary goal.',expectedRevision:1});
    const staleBrief=getAssistantState(await assistantRepo.read());assert.equal(staleBrief.brief?.stale,true);assert.equal(staleBrief.briefs.length,1);
    console.log('PASS: assistant goals, conversation, idempotent answers and retained briefs persist privately without changing lessons; cross-owner injection fails.');
