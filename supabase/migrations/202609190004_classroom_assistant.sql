-- Persist additive assistant state without changing normalized classroom ownership or CAS.
...
begin
  ...
  -- The assistant is owner-scoped by its enclosing classroom. Legacy states may omit it.
  if p_state ? 'assistant' then
    if jsonb_typeof(p_state->'assistant') is distinct from 'object'
      or jsonb_typeof(p_state#>'{assistant,goals}') is distinct from 'object'
      or jsonb_typeof(p_state#>'{assistant,goals,text}') is distinct from 'string'
      or jsonb_typeof(p_state#>'{assistant,turns}') is distinct from 'array'
      or jsonb_typeof(p_state#>'{assistant,briefs}') is distinct from 'array'
      or jsonb_typeof(p_state#>'{assistant,requests}') is distinct from 'array' then
      raise exception 'Invalid assistant state' using errcode='23514';
    end if;
    -- No cross-owner identity can be smuggled into a future nested assistant entity.
    for item in select value from jsonb_path_query(p_state->'assistant', '$.**.ownerId') as matched(value) loop
      if item #>> '{}' is distinct from v_owner::text then raise exception 'Assistant owner mismatch' using errcode='42501'; end if;
    end loop;
    header:=header||jsonb_build_object('assistant',p_state->'assistant');
  end if;
